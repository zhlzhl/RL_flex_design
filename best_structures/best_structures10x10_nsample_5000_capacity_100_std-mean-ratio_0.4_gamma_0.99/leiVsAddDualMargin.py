...
if __name__ == "__main__":
    ...
    sd_d = mean_d*0.4            # standard dAeviation vector (if needed)
    num_samples = 5000
    ...
    flex = np.zeros((n,n))
    num_arcs = int(sorted(best_structures.keys())[-1])
    addDual_allstructs = {}
    for i in range(num_arcs):
        arc_index, perf = add_usedualmargin(flex, cap_vec, demand_train, profit_mat)
        flex[arc_index[0], arc_index[1]]=1
        addDual_allstructs[i+1] = flex.copy()
        print(perf)

    addDual_structs = {}
    for index, struc in best_structures.items():
        addDual_structs[index] = addDual_allstructs[int(index)]
        
    for index, struc in best_structures.items():
        with open("adddual_benchmarks/add_performances_{}T{}"
                      .format(experiment, index), 'wb') as f:
            pickle.dump(addDual_structs[index], f) 

    lei_perfs={}
    adddual_perfs={}
    for index, struc in best_structures.items():
        lei_perfs[index] = evaluate(struc, cap_vec, demand_test, profit_mat)
        adddual_perfs[index] = evaluate(addDual_structs[index], cap_vec, demand_test, profit_mat)
        print("index: {} lei: {} addDual{}".format(index, lei_perfs[index], adddual_perfs[index]))
